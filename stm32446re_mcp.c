...
/*sprintf(buffer, "%u \n",i);
	...
	char i;
I2C3_WriteChar(0x00,0xFF);
I2C3_WriteChar(0x01,0xFF);
while(1){

i=I2C3_ReadChar(0x12);
...
/*void I2C3_WriteChar (char Adr, char Dat)    { 
  ...
  int Status3        = I2C3->SR1;    // read status to clear flag 		
  I2C3->DR           = 0x40;         // slave address   -> DR & write 
  while (!(I2C3->SR1 & 0x0002)) {};  // wait for ADDRESS sent (ADDR=1)
  int Status4        = I2C3->SR1;    // read status to clear flag 		
...
char I2C3_ReadChar (char Adr)    { 
  I2C3->CR1         |= 0x0100;       // send START bit 
  while (!(I2C3->SR1 & 0x0001)) {};  // wait for START condition (SB=1)  
  I2C3->DR           = 0x40;         // slave address -> DR      (LSB=1) 
  while (!(I2C3->SR1 & 0x0002)) {};  // wait for ADDRESS sent    (ADDR=1)  
  ...
  while (!(I2C3->SR1 & 0x0004)) {};  // wait for ByteTransferred (BTF=1)  
	...
	I2C3->CR1         |= 0x0100;       // send START bit 
  while (!(I2C3->SR1 & 0x0001)) {};  // wait for START condition (SB=1) 
  I2C3->DR           = 0x41;         // slave address -> DR      (LSB=0) 
  while (!(I2C3->SR1 & 0x0002)) {};  // wait for ADDRESS sent    (ADDR=1) 
